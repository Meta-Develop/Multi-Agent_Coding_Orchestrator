...
records are deterministic fake evidence generated by `src/evaluation.rs`; no
provider, supervisor, held-out command, or hand-authored plan was executed to
produce them. Cost-shaped values are synthetic fixture data rather than prices,
and no fixture result can authorize production economics or a named default.

The version-1 fixture set contains:

- `hand-authored-plan-v1.json`, the inert plan input whose exact bytes are bound
  by the manifest's SHA-256 digest;
- `manifest-v1.json`, a strict `EvaluationManifest` that binds the goal, exact
