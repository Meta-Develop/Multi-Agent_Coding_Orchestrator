...
| --- | --- | --- |
| Repository-cleanliness capability and primary isolation | Issue #11 (`42f51aa`) capability-binds managed worktree creation. Autopilot checks dirty primary and repository bindings twice; supervise compares its final primary snapshot and fails the run on drift. No result is applied to primary. | `autopilot_rechecks_dirty_primary_immediately_before_supervisor_dispatch`, `dirty_primary_refusal_emits_public_json`, and `fake_autopilot_depth_two_e2e_is_gated_durable_and_primary_untouched` fail if the pre-dispatch guard, typed dirty denial, or HEAD/index/file/status equality regresses. |
...
| Depth, scheduling, journal, and breaker gates | Issues #9 (`1a9642e`, `6bc7e44`), #10 (`9b87d2d` through `61d1179`), and #24 (`d5b46ee`) made these live supervise gates. Autopilot creates exactly one depth-2 plan and calls that public path; plan/depth/admission/journal/breaker errors remain supervisor failures or typed denials. | `fake_autopilot_depth_two_e2e_is_gated_durable_and_primary_untouched` requires a terminal worker, read-only auditor, review-lens aggregate, and durable supervise report. Existing supervise journal/scheduler/breaker tests remain unchanged. |
