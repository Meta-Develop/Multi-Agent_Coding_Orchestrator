...
fn github_pr_with_oversized_changed_files_is_skipped_and_scan_continues() {
    ...
    let normal_item =
        pr_item(normal_raw, &config, &source, &BTreeMap::new()).expect("build normal pr inbox item");
...
fn github_issue_with_oversized_path_proposal_is_skipped_and_scan_continues() {
    ...
    let normal_item =
        issue_item(normal, &config, &source, &BTreeMap::new()).expect("build normal issue inbox item");
