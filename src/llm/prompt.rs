...
impl PromptContext {
    ...
    pub fn assemble_prompt(&self, redactor: &Redactor) -> Prompt {
        ...
        let budget_body = format!(
            ...
            self.provider_capabilities.max_context_tokens.map_or_else(|| "unknown".to_string(), |value| value.to_string()),
            self.provider_capabilities.max_output_tokens.map_or_else(|| "unknown".to_string(), |value| value.to_string()),
