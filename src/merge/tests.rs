...
mod required_network_direct_cancellation_tests {
    ...
    mod network_direct_cancellable_flock_fixture {
        ...
        fn try_nonblocking_exclusive_flock(
            file: &fs::File,
        ) -> Result<NonblockingFlock, std::io::Error> {
...
while :; do sleep 1; done\n",
            ...
            let lock_probe = open_liveness_lock_fd(&liveness_file).unwrap_or_else(|error| {
                panic!("open liveness lock {}: {error}", liveness_file.display())
            });
            ...
            assert!(
                message
                    .starts_with("trusted network cancellable regression process cleanup failed:")
            ...
            let post_join_lock = open_liveness_lock_fd(&liveness_file).unwrap_or_else(|error| {
                panic!("open liveness lock {}: {error}", liveness_file.display())
            });
