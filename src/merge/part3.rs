...
#[cfg(test)]
mod required_network_direct_cancellation_tests {
    use super::*;
    use crate::process_runner::ProcessCancellation;
    use std::collections::BTreeMap;

    #[test]
    fn network_direct_cancellable_matches_validation_failure_of_non_cancellable() {
        let temp = tempfile::tempdir().expect("tempdir");
        let profile = TrustedFixedNetworkProfile::read_write(temp.path())
            .with_resource_limits(Default::default());
        let program = PathBuf::from("/usr/bin/git");
        let args = Vec::new();
        let environment = BTreeMap::new();
        let err_direct = run_required_network_direct(
            "",
            program.clone(),
            args.clone(),
            temp.path(),
        ...
        let err_cancellable = run_required_network_direct_cancellable(
            "",
            program,
            args,
            temp.path(),
        ...
        .expect_err("empty label");
        assert_eq!(err_direct.to_string(), err_cancellable.to_string());
    }

    #[cfg(unix)]
    #[test]
    fn required_direct_cancellable_honours_pre_spawn_cancellation() {
        let temp = tempfile::tempdir().expect("tempdir");
        std::process::Command::new("git")
            ...
            .status()
            .expect("git init");
        let profile = StrictOfflineWorkspaceProfile::read_write(temp.path());
        let environment = capture_git_environment(temp.path()).expect("git environment");
        let cancellation = ProcessCancellation::new();
        cancellation.cancel();
        let error = run_required_direct_with_profile(
            "cancelled required direct",
            resolve_trusted_executable("git").expect("trusted git"),
    ...
    #[cfg(target_os = "linux")]
    mod network_direct_cancellable_flock_fixture {
        use super::*;
        use crate::process_runner::ProcessCancellation;
        ...
        enum NonblockingFlock {
            Acquired,
            WouldBlock,
        }

        fn open_liveness_lock_fd(path: &Path) -> std::io::Result<fs::File> {
            fs::OpenOptions::new()
                .read(true)
                .write(true)
        ...
        fn try_nonblocking_exclusive_flock(file: &fs::File) -> Result<NonblockingFlock, std::io::Error> {
            // SAFETY: flock operates on this live owned descriptor and does not access memory.
            if unsafe { libc::flock(file.as_raw_fd(), libc::LOCK_EX | libc::LOCK_NB) } == 0 {
                return Ok(NonblockingFlock::Acquired);
            }
            let error = std::io::Error::last_os_error();
            let code = error.raw_os_error();
            if code == Some(libc::EWOULDBLOCK) || code == Some(libc::EAGAIN) {
                return Ok(NonblockingFlock::WouldBlock);
            }
            Err(error)
        }

        fn unlock_flock(file: &fs::File) {
            // SAFETY: unlocking this live descriptor is advisory cleanup; closing also releases it.
            let _ = unsafe { libc::flock(file.as_raw_fd(), libc::LOCK_UN) };
        }

        fn descendant_ready_observed(path: &Path) -> bool {
            fs::read(path)
                .map(|bytes| bytes == b"R" || bytes == b"R\n")
                .unwrap_or(false)
        }

        fn assert_descendant_holds_exclusive_lock(file: &fs::File, path: &Path) {
            match try_nonblocking_exclusive_flock(file) {
                Ok(NonblockingFlock::WouldBlock) => {}
                Ok(NonblockingFlock::Acquired) => {
                    unlock_flock(file);
                    panic!(
                        "descendant did not hold exclusive flock on {} before cancellation",
                        path.display()
                    );
                }
                Err(error) => panic!("nonblocking flock on {}: {error}", path.display()),
            }
        }

        fn assert_post_join_lock_released(file: &fs::File, path: &Path) {
            match try_nonblocking_exclusive_flock(file) {
                Ok(NonblockingFlock::Acquired) => unlock_flock(file),
                Ok(NonblockingFlock::WouldBlock) => panic!(
                    "setsid descendant still holds exclusive flock on {} after runner join",
                    path.display()
                ),
                Err(error) => panic!(
                    "nonblocking flock on {} after runner join: {error}",
                    path.display()
                ),
            }
        }

        /// Unwind cleanup only: never panic; report errors without masking the original failure.
        fn wait_lock_released_bounded(
            path: &Path,
            deadline: std::time::Instant,
        ) -> Result<(), String> {
            while std::time::Instant::now() < deadline {
                match open_liveness_lock_fd(path) {
                    Ok(file) => match try_nonblocking_exclusive_flock(&file) {
                        Ok(NonblockingFlock::Acquired) => {
                            unlock_flock(&file);
                            return Ok(());
                        }
            ...
            Err("timed out waiting for descendant flock release during emergency cleanup".into())
        }

        struct EmergencyReleaseGuard {
            emergency_file: PathBuf,
            liveness_file: PathBuf,
            proof_complete: bool,
        }

        impl EmergencyReleaseGuard {
            fn new(emergency_file: PathBuf, liveness_file: PathBuf) -> Self {
                Self {
                    emergency_file,
                    liveness_file,
            ...
            fn mark_proof_complete(&mut self) {
                self.proof_complete = true;
            }
        }

        impl Drop for EmergencyReleaseGuard {
            fn drop(&mut self) {
                if self.proof_complete {
                    return;
                }
                let release_err = fs::write(&self.emergency_file, b"go\n").err();
                let lock_err = wait_lock_released_bounded(
                    &self.liveness_file,
                    std::time::Instant::now() + CLEANUP_BOUND,
                )
                .err();
                if release_err.is_some() || lock_err.is_some() {
                    eprintln!(
                        "network cancellable fixture emergency cleanup: release={release_err:?} flock={lock_err:?}"
                    );
                }
            }
        }
        ...
        struct OwnedRunner {
            ...
            worker: Option<std::thread::JoinHandle<Result<RequiredCommandOutput>>>,
        }
        ...
        #[test]
        fn required_network_direct_cancellable_cleans_setsid_descendant_after_post_spawn_cancel() {
            let temp = tempfile::tempdir().expect("tempdir");
            let liveness_file = temp.path().join("descendant-liveness");
            write_private_file(&liveness_file, b"").expect("liveness file");
            let descendant_ready_marker = temp.path().join("descendant-ready");
            let emergency_file = temp.path().join("emergency-release");
            let mut emergency_guard =
                EmergencyReleaseGuard::new(emergency_file.clone(), liveness_file.clone());
            ...
            let mut environment = minimal_network_environment().expect("network environment");
            environment.insert(
                "GIT_CONFIG_GLOBAL".to_string(),
            ...
            let parent_ready = temp.path().join("child-ready");
            let script = temp.path().join("cancellable-network-fixture.sh");
            write_private_file(
                &script,
                format!(
                    "#!/bin/sh\n\
setsid sh -c 'exec 3<>\"{liveness}\"; flock -x 3 || exit 1; printf R >\"{descendant_ready_marker}\"; while [ ! -e \"{emergency}\" ]; do sleep 0.01; done' &\n\
touch \"{parent_ready}\"\n\
trap '' TERM\n\
while :; do sleep 1; done\n",
                    liveness = liveness_file.display(),
                    descendant_ready_marker = descendant_ready_marker.display(),
                    emergency = emergency_file.display(),
                    parent_ready = parent_ready.display(),
                )
                .as_bytes(),
            )
            .expect("write network cancellation fixture");
            ...
            let args = vec![
                ...
                OsString::from(format!(
                    "alias.maco-network-cancel=!{}",
                    script.to_string_lossy()
                )),
                OsString::from("maco-network-cancel"),
            ];
            let workdir = temp.path().to_path_buf();
            let worker_cancellation = cancellation.clone();
            let worker = std::thread::spawn(move || {
                run_required_network_direct_cancellable(
                    "trusted network cancellable regression",
                    program,
                    args,
                    &workdir,
                    environment,
            ...
            let mut owned = OwnedRunner {
                cancellation,
                worker: Some(worker),
            };

            let ready_deadline = std::time::Instant::now() + READY_BOUND;
            while std::time::Instant::now() < ready_deadline
                && !(parent_ready.exists() && descendant_ready_observed(&descendant_ready_marker))
                && !owned
                    .worker
                    .as_ref()
            ...
            let descendant_ready = descendant_ready_observed(&descendant_ready_marker);
            if !parent_ready.exists() || !descendant_ready {
                owned.cancellation.cancel();
                let result = owned.worker.take().expect("runner thread").join();
                panic!(
                    "network cancellation fixture not ready: parent={} descendant={descendant_ready}; runner={result:?}",
                    parent_ready.exists()
                );
            }

            let lock_probe = open_liveness_lock_fd(&liveness_file)
                .unwrap_or_else(|error| panic!("open liveness lock {}: {error}", liveness_file.display()));
            assert_descendant_holds_exclusive_lock(&lock_probe, &liveness_file);
            ...
            let error = result.expect_err("post-spawn cancellation must fail closed");
            let message = error.to_string();
            assert!(
                message.starts_with("trusted network cancellable regression process cleanup failed:")
                    && message.contains("cancelled by its run supervisor"),
                "{error}"
            );
            assert!(
                join_started.elapsed() < READY_BOUND,
                "runner join exceeded bound after cancellation: {:?}",
                join_started.elapsed()
            );

            let post_join_lock = open_liveness_lock_fd(&liveness_file)
                .unwrap_or_else(|error| panic!("open liveness lock {}: {error}", liveness_file.display()));
            assert_post_join_lock_released(&post_join_lock, &liveness_file);
            emergency_guard.mark_proof_complete();
        }
    }
}
